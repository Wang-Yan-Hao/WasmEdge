...
int main() {
  ...
  WasmEdge_String ProviderName = WasmEdge_StringCreateByCString("module_2");

  WasmEdge_Result Res = WasmEdge_VMRegisterModuleFromFile(
      VMCxt, ProviderName, "./wasm_src/provider.wasm");
  ...
  Res = WasmEdge_LoaderParseFromFile(LoadCxt, &ASTCxt,
                                     "./wasm_src/consumer.wasm");
  if (!WasmEdge_ResultOK(Res)) {
