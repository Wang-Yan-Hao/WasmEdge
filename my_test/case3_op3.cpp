...
int main() {
  ...
  std::cout << "Operation: Delete Host's direct reference..." << std::endl;
  WasmEdge_ModuleInstanceDelete(HostMod2);

  std::cout << "Checking survival: VM1 should still be able to run."
            << std::endl;
  run_registered_func(VM1, "module_1", "run");

  // Actually delete
  WasmEdge_VMDeleteRegisteredModule(VM1, Mod1Name);
  WasmEdge_VMDeleteRegisteredModule(VM2, Mod4Name);
}
