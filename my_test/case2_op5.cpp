...
int main() {
  ...
  WasmEdge_Value Returns[1];
  WasmEdge_Result Res =
      WasmEdge_ExecutorInvoke(Exec, FuncInst, Params, 2, Returns, 1);
  if (WasmEdge_ResultOK(Res)) {
    std::cout << "No Store still can run" << std::endl;
  }
}
