...
TEST(APICoreTest, VMDeleteRegistered) {
...

  {
    ...
    Res = WasmEdge_VMRegisterModuleFromBuffer(
        VMCxt, PName, Provider1Wasm.data(), Provider1Wasm.size());
    EXPECT_TRUE(WasmEdge_ResultOK(Res));

    // Register Consumer 1 Linked to Provider 1
    Res = WasmEdge_VMRegisterModuleFromBuffer(
        VMCxt, CName1, ConsumerWasm.data(), ConsumerWasm.size());
    EXPECT_TRUE(WasmEdge_ResultOK(Res));
    ...
    Res = WasmEdge_VMRegisterModuleFromBuffer(
        VMCxt, PName, Provider2Wasm.data(), Provider2Wasm.size());
    EXPECT_TRUE(WasmEdge_ResultOK(Res));
    EXPECT_EQ(WasmEdge_VMListRegisteredModuleLength(VMCxt), OriginalCount);

    // Register Consumer 2 Linked to Provider 2
    WasmEdge_VMRegisterModuleFromBuffer(VMCxt, CName2, ConsumerWasm.data(),
                                        ConsumerWasm.size());
...

  {
    ...
    Res = WasmEdge_VMLoadWasmFromBuffer(VMCxt, ConsumerWasm.data(),
                                        ConsumerWasm.size());
    EXPECT_TRUE(WasmEdge_ResultOK(Res));
...

  {
    ...
    // Register Provider 1
    Res = WasmEdge_VMRegisterModuleFromBuffer(VM1, PName, Provider1Wasm.data(),
                                              Provider1Wasm.size());
    EXPECT_TRUE(WasmEdge_ResultOK(Res));

    // Register Consumer 1 Linked to Provider 1
    Res = WasmEdge_VMRegisterModuleFromBuffer(VM2, CName1, ConsumerWasm.data(),
                                              ConsumerWasm.size());
    EXPECT_TRUE(WasmEdge_ResultOK(Res));
...

  {
    ...
    Res = WasmEdge_VMRegisterModuleFromBuffer(VM1, CName1, ConsumerWasm.data(),
                                              ConsumerWasm.size());
    EXPECT_TRUE(WasmEdge_ResultOK(Res));
    Res = WasmEdge_VMRegisterModuleFromBuffer(VM2, CName2, ConsumerWasm.data(),
                                              ConsumerWasm.size());
    EXPECT_TRUE(WasmEdge_ResultOK(Res));
...

  {
    ...
    WasmEdge_VMDelete(VM1);
    // The provider module can't be find in StoreManager but stil alive
    EXPECT_EQ(WasmEdge_StoreFindModule(SharedStore, PName), nullptr);
 ...
 * Cases:
 * 1. Delete a shared store while active module dependencies exist.
 * 2. Release host ownership of a module shared across multiple independent VMs.
 * 3. Release host ownership of a provider module within a shared store VM.
 */
TEST(APICoreTest, ModuleDeletion) {
  ...
  {
    // Case 1: Delete a shared store while active module dependencies exist.
  ...
  {
    // Case 2: Release host ownership of a module shared across multiple
  ...
  {
    // Case 3: Release host ownership of a provider module within a shared store
    // VM. Verify that WasmEdge_ModuleInstanceDelete unlinks the module from the
    // common registry while preserving the instance for all consumers across
    // different VMs sharing that store.
    WasmEdge_StoreContext *SharedStore = WasmEdge_StoreCreate();
    WasmEdge_VMContext *VM1 = WasmEdge_VMCreate(nullptr, SharedStore);
    WasmEdge_VMContext *VM2 = WasmEdge_VMCreate(nullptr, SharedStore);
